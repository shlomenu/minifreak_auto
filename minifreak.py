...
class MiniFreak:
    ...
    def _add_programmed_action(self, afford, *conditions, **kwargs):
        if   afford is self.aff.midi_disc_toggle:
            self._midi_disc_toggle[conditions[0].name] = {
                "kwargs": kwargs,
            }
        elif afford is self.aff.midi_slider:
            kwargs.update(control=conditions[0].value)
            self._midi_slider[conditions[0].name] = {
                "kwargs": kwargs,
            }
        elif afford is self.aff.midi_cont_toggle:
            self._midi_cont_toggle[conditions[0].name] = {
                "kwargs": {"control": conditions[0].value},
            }

    def _add_ui_action(self, aff, *conditions):
        def norm(x, y):
            return (
    ...
    def _finalize_actions(self):
        action_types, self._spec, self._state, self._states = {}, {}, {}, {}
        for aff in self.aff:
            key = "_".join(map(lambda s: s.capitalize(), aff.name.split("_")))
            records = getattr(self, f"_{aff.name}")
            action_types[key] = Enum(key, tuple(records.keys()))
            for subkey in action_types[key]:
                elt = records[subkey.name]
                self._spec[subkey] = elt if Def_Afford.is_ui(aff) else elt["kwargs"]  
                if   aff is self.aff.midi_disc_toggle or aff is self.aff.midi_cont_toggle:
                    self._spec[subkey]["toggle"] = Toggle_State.On
                    self._state[subkey] = Toggle_State.Off
                    self._states[subkey] = tuple(Toggle_State)
                elif aff is self.aff.click_select:
                    self._state[subkey] = elt["selections"][0]
                    self._states[subkey] = tuple(elt["selections"])
                elif aff is self.aff.click_toggle or aff is self.aff.click_hold_toggle:
                    self._state[subkey] = Toggle_State.Off
                    self._states[subkey] = tuple(Toggle_State)
        ...
        self._action_types = Enum('Action_Types', action_types)
        self._actions = tuple((a for at in self._action_types for a in at))
    ...
    def do(self, action, **user_kwargs):
        at = self._action_types[action.__name__]
        if at is self._action_types.Midi_Disc_Toggle:
            kwargs = self._spec[action]
            kwargs.update(user_kwargs)
            if kwargs["toggle"] is Toggle_State.On:
                del kwargs["toggle"]
                self._send(f"{action.name}_on", **kwargs)
            elif user_kwargs["toggle"] is Toggle_State.Off:
                del kwargs["toggle"]
                self._send(f"{action.name}_off", **kwargs)            
        if at is self._action_types.Midi_Cont_Toggle:
            kwargs = self._spec[action]
            kwargs.update(user_kwargs)
            if kwargs["toggle"] is Toggle_State.On:
                del kwargs["toggle"]
                self._send("control_change", value=127, **kwargs)
            if kwargs["toggle"] is Toggle_State.Off:
                del kwargs["toggle"]
                self._send("control_change", value=0, **kwargs)
        if at is self._action_types.Midi_Slider:
            kwargs = self._spec[action]
            self._send("control_change", **kwargs)
        if at is self._action_types.Click_Select:
            pass
        if at is self._action_types.Click_Toggle:
            pass
        if at is self._action_types.Click_Hold_Toggle:
            pass
        if at is self._action_types.Click_Refresh:
            pass
        if at is self._action_types.Click_Cycle:
            pass
        if at is self._action_types.Click_Dropdown:
            pass
        if at is self._action_types.Click_Rel_Slider:
            pass
